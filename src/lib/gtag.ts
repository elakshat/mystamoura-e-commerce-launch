...
// Measurement ID is loaded from site_settings (database) at runtime,
// NOT from environment variables (which aren't available in frontend builds).
...
/**
 * Set the measurement ID (called from RouteTracker after fetching from DB).
 * Then inject the gtag script.
 */
export function initGA(measurementId?: string) {
  const id = measurementId || (import.meta.env.VITE_GA_MEASUREMENT_ID as string | undefined);
  if (!id) return;
  if (GA_MEASUREMENT_ID === id) return; // already initialised with same ID
